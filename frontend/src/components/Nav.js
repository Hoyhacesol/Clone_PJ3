...
const Nav = ({ onChatClick, chatOpen, unreadCount }) => {
  ...
  const handleLogout = async (e) => {
    e.preventDefault();
    try {
      await fetch("http://43.201.249.236:8080/logout", {
        method: "POST",
        credentials: "include",
      });
    ...
    <>
          ...
          {!user ? (
            ...
            <div className={styles.user_dropdown}>
              ...
              <div className={`${styles.user_dropdown_box} ${dropdownOpen ? styles.open : ""}`}>
                <img className={styles.profile_img_dropdown} src={`http://43.201.249.236:8080/images/${user.profileimg || "baseprofile.png"}`} alt="프로필 사진" />
                <p>
                  안녕하세요! <br />
      ...
      <div className={`${styles.sidebar} ${isSidebarOpen ? styles.show : ""}`}>
        <div className={styles.sidebar_userbox}>
          {!user ? (
            <>
              <img className={styles.profile_img} src={`http://43.201.249.236:8080/images/baseprofile.png`} alt="비로그인 프로필 사진" />
              <p>비로그인 상태입니다.</p>
              <div className={styles.sidebar_buttons}>
            ...
            <>
              <img className={styles.profile_img} src={`http://43.201.249.236:8080/images/${user.profileimg || "baseprofile.png"}`} alt="프로필 사진" />
              <p>
                안녕하세요! <br />
                <strong>{user.nickname}</strong> 님
              </p>
