...
function SignupPageSeller() {
  ...
  const handleNicknameCheck = async () => {
    try {
      const res = await fetch(`http://43.201.249.236:8080/api/members/nickname_check?nickname=${encodeURIComponent(nickname)}`, {
        method: "GET",
        credentials: "include",
      });
  ...
  const handleSendSMS = async () => {
    ...
    try {
      const res = await axios.post(
        "http://43.201.249.236:8080/api/sms/send",
        {
          phone: cleanedPhone,
        },
  ...
  const handleVerifySMS = async () => {
    ...
    try {
      const res = await axios.post(
        "http://43.201.249.236:8080/api/sms/verify",
        {
          phone: cleanPhone,
          code: authCode,
        },
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post("http://43.201.249.236:8080/api/members/signupseller", {
        userId: email,
        userName: nickname,
        userPw: password,
        phone: phone,
