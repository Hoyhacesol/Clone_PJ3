...
const MyPage = () => {
  ...
  const handleRoleToggle = async () => {
    if (user.role === "USER") {
      // user에서 seller로
      try {
        const res = await axios.get(`/api/members/checkseller`, {
          params: { mno: user.mno },
        });

        if (res.data.exists) {
          // 기존 seller데이터 존재
          await axios.patch("/api/members/changetoseller", null, {
            params: {
              mno: user.mno,
            },
          });
      ...
      // seller에서 user로
      await axios.patch("/api/members/changetouser", null, {
        params: {
          mno: user.mno,
        },
      });
    ...
    <div className={styles.mypage_container}>
      ...
      <section className={styles.mypage_section}>
        ...
        <div className={styles.profile_image_wrapper}>
          <img key={user.profileimg} src={`/images/${user.profileimg || "baseprofile.png"}?t=${new Date().getTime()}`} alt="프로필 이미지" className={styles.profile_image} />
        </div>
